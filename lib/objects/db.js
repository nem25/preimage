...
class Db {
  ...
  async findPaymentsByRecipientBy (input) {
    ...
    const sponsor = await Sponsor.findOne(input)
    if (!owner && !sponsor) return []
    const orClause = []
    if (owner) orClause.push({ 'recipient.item': owner._id })
    if (sponsor) orClause.push({ 'recipient.item': sponsor._id })
  ...
  async deletePaymentsByRecipientBy (input) {
    const owner = await Owner.findOne(input)
    const sponsor = await Sponsor.findOne(input)
    if (!owner && !sponsor) return []
    const orClause = []
